...
positive cases, with a\@perumalsamy2024dditional processing to resolve
inconsistencies in timelines and reporting. Exploratory data analysis
(EDA) was used to examine disease distribution, demographic patterns,
geographic hotspots, and data completeness over time. Building on these
insights, predictive models were developed to assess whether patient and
case-level factors—such as age, disease type, and county—could help
identify cases at risk of delayed follow-up. This work theorectically
supports improved understanding of disease surveillance data and
highlights areas for strengthening tick-borne disease reporting and case
...
the globe [@mcpherson2017]. Researchers have documented the ecology and
behavior of ticks, explored the role of climate and land use in tick
habitat expansion, and analyzed patterns of disease incidence in both
human and animal populations. Despite the growing body of research, many
knowledge gaps remain, particularly in understanding co-infections,
long-term health effects, and effective prevention strategies
[@usepa2016]. Tick-borne diseases (TBDs) are primarily transmitted
through the bite of infected hard-bodied ticks (family Ixodidae), and
include bacterial, viral, and protozoan pathogens. Today, TBDs are
recognized as a major public health concern in both rural and suburban
settings, with transmission risk influenced by ecological, climatic, and
sociocultural factors [@perumalsamy2024].
...
ticks expand due to environmental and ecological changes @cdc2024a.
...
temperature, humidity, vegetation, and host availability[@berger2014;
@bouchard2019]. Human exposure risk is increased in areas with high host
density, edge habitats between forest and residential zones, and during
seasons with peak tick activity [@lerman2018; @aronson2017].
Additionally, land use changes—such as suburban development,
deforestation, and agricultural expansion—can modify tick habitats and
affect pathogen dynamics @allan2003.
...
[@kasari2010; @clark]. From a public health perspective, TBDs can cause
a range of acute and chronic symptoms in humans, from fever and fatigue
to long-term neurological and musculoskeletal complications
@zychowski2024. Co-infections—when an individual is infected with more
than one pathogen—are increasingly recognized and may complicate
diagnosis and treatment. The public health burden is also shaped by
disparities in access to health care, awareness of TBDs, and
availability of diagnostic testing, which may result in underreporting
or misdiagnosis. The costs associated with tick-borne disease—including
medical care, productivity loss, and prevention efforts—represent a
growing concern for health systems, particularly in endemic areas
@berghoff2012.
...
disease transmission [@gage2008; @dawe2016]. In the southeastern U.S.,
for example, Amblyomma americanum (the lone star tick) has expanded in
abundance and geographic range over the past decades, contributing to
increased cases of ehrlichiosis and the emergence of Alpha-Gal Syndrome,
a red meat allergy triggered by tick bites @cdc2022. Understanding the
ecology, distribution, and drivers of tick-borne disease is therefore
crucial to predicting future trends, identifying at-risk populations,
and developing effective prevention and response strategies.
...
## EDA Relevant Takeaways

#### Most Common Tick-Borne Diseases

The data set is filtered to identify the five most frequently reported
...
**Figure 1.** Distribution of tick-borne disease case counts in Georgia,
2019–2024.
...
**Figure 2-4.** The distribution of the most common disease types across
all counties– narrowed to the counties with more tick cases (hotspot
counties) then further narrowed focus on the top five tick-borne
diseases within these hotspot counties
...
**Figure 5.** Data completeness variation in the data set and entry
...
#### Timeline Analysis of Key Case Events
...
+----------------+-------------+-----------+-----------+-----------+
| term\          | estimate\   | std.      | st        | p.value\  |
| \<chr\>        | \<dbl\>     | error\    | atistic\  | \< dbl\>  |
|                |             | \<dbl\>   | \<dbl\>   |           |
+:===============+============:+==========:+==========:+==========:+
| (Intercept)    | \-          | 157       | 1.        | 0         |
|                | 2           | 3.052665  |  48025251 | .1729390  |
|                | 328.5151515 |           |           |           |
+----------------+-------------+-----------+-----------+-----------+
| YEAR           | 1.1515152   | 0.777581  | 1.        | 0         |
|                |             |           |  48089418 | .1727716  |
+----------------+-------------+-----------+-----------+-----------+
| BABESIOSIS     | 0.4242424   | 5 .484565 | 0 .       | 0         |
|                |             |           | 07735207  | .9400358  |
+----------------+-------------+-----------+-----------+-----------+
| DENGUE         | 4.8484848   | 6\.       | 0 .       | 0         |
|                |             | 364775    | 76176850  | .4656907  |
+----------------+-------------+-----------+-----------+-----------+
| EHRLICHIA      | 5.0000000   | 5.157889  | 0 .       | 0         |
| CHAFFEENSIS    |             |           | 96938889  | .3576794  |
+----------------+-------------+-----------+-----------+-----------+
| EHRLICHIA      | \- 1 .      | 6.364775  | 0 .       | 0         |
| EWINGII        | 1515152     |           | 18092002  | .8604390  |
+----------------+-------------+-----------+-----------+-----------+
| LYME DISEASE   | 8.5606061   | 4.963745  | 1 .       | 0         |
|                |             |           | 72462659  | .1186814  |
+----------------+-------------+-----------+-----------+-----------+
| ROCKY MOUNTAIN | 0.5757576   | 5.484565  | 0\.       | 0         |
| SPOTTED FEVER  |             |           | 10497781  | .9186958  |
+----------------+-------------+-----------+-----------+-----------+
| WEST NILE      | 8.2272727   | 5.593721  | 1.        | 0         |
| (WNV)          |             |           |  47080497 | .1754208  |
| INFECTION      |             |           |           |           |
+----------------+-------------+-----------+-----------+-----------+

9 rows

**Table 1.** Output form a linear model predicting total case counts by
year and disease type. The intercept represents baseline case counts
while each diseases represents additional effect size.
...
+---------+---------+---------+---------+---------+
| term    | e       | std .   | s t a t | p .     |
|         | stimate | error   | istic   | value   |
| \       |         |         |         |         |
| \       | \       | \       | \       | \       |
| \<chr\> | \       | \       | \       | \       |
|         | \<      | \<      | \<      | \<      |
...
+:========+========:+========:+========:+========:+
| ( I n   | 9935 .  | 5729 .  | 1 .     | 0 .     |
| t       | 267857  | 946482  | 733920  | 1434648 |
| ercept) |         |         |         |         |
+---------+---------+---------+---------+---------+
| YEAR    | 4 .     | 2 .     | 1 .     | 0 .     |
|         | 910714  | 835202  | 732051  | 1438108 |
+---------+---------+---------+---------+---------+
...
## Adding in Poverty Rate Data by County
...
surveillance data. These results align with previous findings that
individual-level demographics and geography can influence diagnosis of
vector-borne diseases, though the moderate predictive power suggests
additional factors— such as healthcare access and proximity
[@starke2023; @frontier]— remain unmeasured in this simplistic modeled
understanding of time to diagnosis. Such insights can inform
improvements in case monitoring, triaging, and outreach in counties or
patient groups with higher risk for delays.
...
model indicated no statistically significant relationshipbetween poverty
rate and delay rate (p = 0.87). The estimated effect size was minimal (β
= 0.0049), and the 95% confidence interval included zero.
...
-   The all-predictors model performed more balanced, with a decent
    accuracy and a stronger ROC AUC, meaning it did a better job
    discriminating between delayed and non-delayed cases.
